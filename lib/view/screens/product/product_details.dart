...
import 'package:fmtmimi/services/localization_services.dart';
import 'package:fmtmimi/view/screens/cart/cart_screen.dart';
...
class YourPageState extends State<ProductDetailsScreen>
  ...
  void initState() {
    ...
    Get.find<ProductDetailsController>()
        .addedSpecification(product: widget.productDetails!);
          ...
          InkWell(
            onTap: () async {
              if (OthersMethod.variationsIsEmptyCheck(
                ...
                int cartQuantity = Get.find<CartController>()
                    .cartItemQuantity(productDetailsController.id ?? 0);
                log('$cartQuantity');
                if (cartQuantity <
                    (productDetailsController.stockQuantity ?? 0)) {
                  Get.find<CartController>().addToCart(
                    productDetailsController,
                    price: OthersMethod.productPriceCalculate(
                        productDetailsController),
                    quantity: 1,
                    brand:
                        OthersMethod.selectedBrands(productDetailsController),
                  );
                } else {
                  showCustomSnackBar(
                    'No more stock!'.tr,
