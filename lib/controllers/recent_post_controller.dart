...
import 'package:fmtmimi/helper/sqlite_db_helper.dart';
import 'package:fmtmimi/models/response/product_list.dart';
import 'package:fmtmimi/models/response/recents_model.dart';
...
class RecentController extends GetxController {
  ...
  void addToRecent(ProductList tempModel) async {
    ...
    if (isExist.isNotEmpty) {
      ...
      if (_recentList.isNotEmpty && 20 < _recentList.length) {
        //21 ar opr hole
  ...
  Future<void> removeFromRecent(
    ...
    try {
      ...
      showCustomSnackBar(
        'Remove from recent successfully!',
      );
      getRecentList();
    } finally {
      _isLoading = false;
  ...
  void emptyRecentView() async {
    try {
      ...
      getRecentList();
      Get.back();
      showCustomSnackBar('Clear item form recent view successfully!',
          isError: false);
    } finally {
      _isLoading = false;
      update();
